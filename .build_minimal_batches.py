...
def create_batch(path, include_source_rows, append_samples):
    ...
    ws = wb.create_sheet('Isolates')
    rows = list(src_iso.iter_rows(values_only=True))
    ...
    ws = wb.create_sheet('__shadow__')
    rows = list(src_shadow.iter_rows(values_only=True))
    ws.append(list(rows[0]))
    if include_source_rows:
        for row in rows[1:]:
            ws.append(list(row))
    ...
    # copy the remaining metadata sheets verbatim only for batch_04; batch_05 gets headers and a light copy.
...
create_batch(out_04, True, fill_04)
create_batch(out_05, False, fill_05)
