...
missing = [
    'Flu-0583','Flu-0584','Flu-0586','Flu-0589','Flu-0592','Flu-0593','Flu-0596',
    'Flu-0599','Flu-0608','Flu-0611','Flu-0613','Flu-0614','Flu-0615','Flu-0619',
    'Flu-0621','Flu-0622','Flu-0623','Flu-0641','Flu-0652','Flu-0654','Flu-0658'
]
fill_04 = missing[:11]
fill_05 = missing[11:]
...
style_cache = []
for c in range(1, iso.max_column + 1):
    cell = iso.cell(iso_template, c)
    style_cache.append((copy(cell._style), copy(cell.font), copy(cell.fill), copy(cell.border), copy(cell.alignment), cell.number_format, copy(cell.protection)))
shadow_cache = []
for c in range(1, shadow.max_column + 1):
    cell = shadow.cell(shadow_template, c)
    shadow_cache.append((copy(cell._style), copy(cell.font), copy(cell.fill), copy(cell.border), copy(cell.alignment), cell.number_format, copy(cell.protection)))
...
def build_row(sample):
    ...
    return [data.get(h) for h in headers], data['Segment_Ids']


def append_isolate(ws, values):
    r = ws.max_row + 1
    for c, value in enumerate(values, start=1):
        dst = ws.cell(r, c)
        style = style_cache[c - 1]
        dst._style = copy(style[0])
        dst.font = copy(style[1])
        dst.fill = copy(style[2])
        dst.border = copy(style[3])
        dst.alignment = copy(style[4])
        dst.number_format = style[5]
        dst.protection = copy(style[6])
        dst.value = value


def append_shadow(ws, sample, segs):
    r = ws.max_row + 1
    vals = [sample, segs]
    for c, value in enumerate(vals, start=1):
        dst = ws.cell(r, c)
        style = shadow_cache[c - 1]
        dst._style = copy(style[0])
        dst.font = copy(style[1])
        dst.fill = copy(style[2])
        dst.border = copy(style[3])
...
def append_sequences(ws, sample):
    for seg in [seg for seg in segment_order if seg in seqs.get(sample, {})]:
        ws.append([f'>{sample}|A_{seg}'])
        ws.append([seqs[sample][seg]])

for sample in fill_04:
    values, segs = build_row(sample)
    append_isolate(iso, values)
    append_shadow(shadow, sample, segs)
    append_sequences(seqsheet, sample)
wb.save(base_batch)
wb.close()
