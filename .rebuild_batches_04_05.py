...
def copy_full_sheet(src_ws, dst_ws):
    for row in src_ws.iter_rows(values_only=True):
        dst_ws.append(list(row))


def copy_with_header(src_ws, dst_ws, append_rows=None):
    rows = list(src_ws.iter_rows(values_only=True))
    if not rows:
        return
    dst_ws.append(list(rows[0]))
    for row in rows[1:]:
        dst_ws.append(list(row))
    if append_rows:
...
def write_workbook(target_path, include_source_isolates, append_samples):
    src_wb = load_workbook(source_path, read_only=True, data_only=False)
    ...
    for sname in src_wb.sheetnames:
        ...
        if sname == 'Isolates':
            ...
            for sample in append_samples:
                ...
                dst_ws.append([sample, ','.join(segs)])
        elif sname == 'Sequences':
            if include_source_isolates:
                copy_full_sheet(src_ws, dst_ws)
            for sample in append_samples:
                _, segs = build_row(sample, summary, origin, seqs)
                for seg in segs:
                    dst_ws.append([f'>{sample}|A_{seg}'])
                    dst_ws.append([seqs[sample][seg]])
        else:
            copy_full_sheet(src_ws, dst_ws)
    src_wb.close()
    wb.save(target_path)
...
write_workbook(source_path, True, fill_04)
write_workbook(out_05, False, fill_05)
print('done')
