#!/usr/bin/env python3
"""Build metadata/H5N1_context.csv: panel + Ecuador subset + segment/lineage matrices."""
    ...
    for isolate, segs in sorted(complete_context.items()):
        for _seg, (epi_isl, _seq, _hdr) in segs.items():
            context_rows.append(
                {
                    "file_name": epi_isl,
                    "collection_date": context_dates[isolate],
                    "country": context_places[isolate],
                    "expected_role": context_types[isolate],
                }
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build metadata/H5N1_context.csv.")
    ...
    epi_to_segment, epi_to_isolate = build_context_epi_maps(complete_context)
    ...
    for panel_row in panel_rows:
        ...
        if file_name in filtrado_by_epi:
            ...
            apply_context_lineages(
                row, file_name, epi_to_segment, epi_to_isolate, lineages_by_epi, lineages_by_strain
            )
    ...
    n_context_si = sum(1 for row in rows if row["file_name"] not in filtrado_by_epi and any(row[seg] == "SI" for seg in SEGMENTS))
    n_context_lin = sum(
    ...
    print(
        ...
        f"(Ecuador={n_ecuador}, context={len(rows) - n_ecuador}, "
