...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build concatenated codon-aware partitions for validation runs"
    )
    parser.add_argument(
        "--segment-order",
        required=True,
    ...
    for segment in segment_order:
        ...
        if segment in codon_segments:
            ...
            partition_lines.append(
                f"DNA, {segment}_cp12 = {cp1}-{end}\\3,{cp2}-{end}\\3"
            )
            partition_lines.append(f"DNA, {segment}_cp3 = {cp3}-{end}\\3")
        else:
    ...
    print(f"Wrote concatenated validation alignment: {args.output_alignment}")
    print(f"Wrote codon-aware validation partitions: {args.output_partitions}")
